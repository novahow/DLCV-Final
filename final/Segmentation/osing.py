...
def step1_python_tianchi(case_path):
    ...
    sliceim,origin,spacing,isflip = load_itk_image(case_path)
    if isflip:
        sliceim = sliceim[:,::-1,::-1]
        print('flip!')
...
def preprocess_luna():
    ...
    if 1:#not os.path.exists(finished_flag):
        annos = np.array(pandas.read_csv(luna_label))
        pool = Pool()
        if not os.path.exists(savepath):
            os.mkdir(savepath)
        # print('process subset', setidx)
        for setidx in ['train', 'test']:
            # print 'process subset', setidx
            filelist = [f.split('.mhd')[0] for f in os.listdir(join(luna_data, setidx)) if f.endswith('.mhd') ]
            if not os.path.exists(join(savepath, str(setidx))):
                os.mkdir(join(savepath, str(setidx)))
            partial_savenpy_luna = partial(savenpy_luna, annos=annos, filelist=filelist,
                                       luna_segment=luna_segment, luna_data=join(luna_data, setidx), 
                                       savepath=join(savepath, setidx))
            N = len(filelist)
            #savenpy(1)
...
def gsl(slice):
    selem = disk(10)
    binary = binary_closing(slice, selem)
...
def singp(n, path, mode, offset):
    print(n)
    img, b1, b2, spc, ori, _ = step1_python_tianchi(join(path, n))
    mhd = sitk.ReadImage(join(path, n))
    nimg = np.copy(img)
    lr, rb = np.array([gsl(slice) for slice in b1]), np.array([gsl(slice) for slice in b2])
    nimg[lr] = 8000
    nimg[rb] = 8001
    nimg[nimg < 8000] = 0
    nimg[nimg == 8000] = 3
    nimg[nimg == 8001] = 4 
    nimg = nimg.astype('short')
    img = sitk.GetImageFromArray(nimg)
    ...
    print('imgok')
    sitk.WriteImage(img, join(join(join(offset, 'nseg'), mode), n), useCompression = True)
...
if __name__ == '__main__':
    ...
    singp(args.name, args.path, args.mode, './data/')