...
def ns(n, path, mode, offset):
    img, ori, spc = load_itk_image(join(path, n))
    itk = sitk.ReadImage(join(path, n))
    print(n, img.shape)
    bm1, bm2, l = extract_lung(img, spc)
    nimg = np.copy(img)
    lr, rb = np.array([gsl(slice) for slice in bm1]), np.array([gsl(slice) for slice in bm2])
    nimg[lr] = 8000
    nimg[rb] = 8001
    nimg[nimg < 8000] = 0
    nimg[nimg == 8000] = 3
    ...
    img.SetSpacing(itk.GetSpacing())
    img.SetOrigin(itk.GetOrigin())
    img.SetDirection(itk.GetDirection())
    print('imgok')
