...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None):
        ...
        for idx in idcs:
            # print data_dir, idx
            l = np.load(os.path.join(data_dir, idx) + '_label.npy', allow_pickle=True)
            # print l, os.path.join(data_dir, '%s_label.npy' %idx)
            if np.all(l == 0):
                l = np.array([])
            labels.append(l.astype('float32'))
            # print(l)
        # print(labels)
...
class LabelMapping(object):
    ...
    def __call__(self, input_size, target, bboxes, filename):
        ...
        label = -1 * np.ones(output_size + [len(anchors), 5], dtype=np.float64)
