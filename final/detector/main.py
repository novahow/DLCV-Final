...
parser.add_argument('--save-freq', default=3, type=int, metavar='S',
                    help='save frequency')
...
def main():
    ...
    sch = get_linear_schedule_with_warmup(optimizer, len(train_loader) * args.epochs // 20, len(train_loader) * args.epochs)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        train(train_loader, net, loss, epoch, optimizer, get_lr, args.save_freq, save_dir, sch)
        # validate(val_loader, net, loss)
        # test(test_loader, net, get_pbb, save_dir,config)


def train(data_loader, net, loss, epoch, optimizer, get_lr, save_freq, save_dir, sch):
    ...
    for i, (data, target, coord) in enumerate(data_loader):
        ...
        coord = Variable(coord.cuda()).float()

        output = net(data, coord)
        # print(coord.dtype, type(output))
    ...
    print(('Epoch %03d (lr %.5f)' % (epoch, sch.get_last_lr()[0])))
    ...
    with torch.no_grad():
        ...
        for i_name, (data, target, coord, nzhw) in enumerate(data_loader):
            ...
            for i in range(len(splitlist)-1):
                input = Variable(data[splitlist[i]:splitlist[i+1]]).cuda().float()
                inputcoord = Variable(coord[splitlist[i]:splitlist[i+1]]).cuda().float()
                if isfeat:
            ...
            print('pbbthres', np.max(pbb[:, 0]), np.min(pbb[:, 0]))
            if isfeat:
